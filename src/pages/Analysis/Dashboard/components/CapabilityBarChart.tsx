...
const CapabilityBarChart = ({ rows, className = '' }: CapabilityBarChartProps) => (
  <section className={`bg-surface-container-lowest p-6 sm:p-8 rounded-[14px] shadow-[0px_20px_40px_rgba(10,25,47,0.06)] ${className}`}>
    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
      ...
      <div className="flex gap-4 text-xs font-bold shrink-0">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-on-surface-variant">전공 평균</span>
        </div>
      </div>
    </div>
      ...
      {rows.map((row) => (
        <div key={row.axis} className="space-y-1.5">
          <div className="flex justify-between text-sm font-bold">
            ...
            <span className={`tabular-nums ${row.userScore >= row.majorAvg ? 'text-secondary' : 'text-on-surface-variant'}`}>
              {row.userScore} / {row.majorAvg}
            </span>
          </div>
          <div className="relative h-3 bg-surface-container-low rounded-full overflow-hidden">
            <div
              ...
              style={{ width: `${row.majorAvg}%` }}
            />
            <div
              className={`absolute h-full rounded-full transition-all duration-500 ${row.userScore >= row.majorAvg ? 'bg-secondary' : 'bg-secondary/60'}`}
              style={{ width: `${row.userScore}%` }}
            />
          </div>
        </div>
      ))}
