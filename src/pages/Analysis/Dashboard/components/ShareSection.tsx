          ...
          <div>
            ...
            <div className="glass-panel p-5 sm:p-6 rounded-[14px] border border-white/10 shadow-xl">
              ...
              <div className="flex items-end justify-between gap-4">
                <div>
                  <div className="text-2xl sm:text-3xl font-bold text-white mb-1">
                    {topMajor?.name ?? '추천 전공 없음'}
                  </div>

                  <div className="text-secondary-container font-medium">
                    {topMajor?.category ?? '진단 결과 기반 추천'}
                  </div>
                </div>
                ...
                <div className="text-right shrink-0">
                  ...
                  <div className="text-[10px] text-white/50 uppercase tracking-tighter">
                    Suitability Score
                  </div>
                </div>
              </div>
              ...
              <div className="mt-4 h-1.5 w-full bg-white/10 rounded-full overflow-hidden">
                <div
                  className="h-full bg-secondary-container shadow-[0_0_10px_rgba(0,210,255,0.6)]"
        ...
        <div className="flex-1 relative flex items-center justify-center min-h-[260px] sm:min-h-[320px]">
          ...
          <div className="relative z-10 w-full h-full flex items-center justify-center">
            <svg className="w-full max-h-[320px] radar-glow" viewBox="0 0 400 400">
              ...
              {DOT_COORDS.map(([cx, cy], index) => (
                <circle key={`${cx}-${cy}-${index}`} cx={cx} cy={cy} r="5" fill="#00D2FF" />
              ))}
            </svg>
            ...
            {AXIS_LABELS.map(({ label, cls }) => (
              <div
                key={label}
                className={`absolute text-white/60 text-[10px] sm:text-xs font-bold tracking-widest uppercase ${cls}`}
              >
                {label}
              </div>
            ))}
