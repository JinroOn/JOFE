...
const RadarChart = ({
  ...
  const dataPoints = (scores: number[]) =>
    scores.map((s, i) => getPoint(r * s, i))
      .map(p => `${p.x.toFixed(2)},${p.y.toFixed(2)}`)
      .join(' ');
  ...
  return (
    <svg viewBox="0 0 240 240" className="w-full h-full" style={{ overflow: 'visible' }}>
      ...
      {Array.from({ length: n }, (_, i) => {
        ...
        return (
          <line key={i} x1={cx} y1={cy} x2={end.x} y2={end.y} stroke="#c5c6cd" strokeWidth={0.5} />
        );
      })}
      <polygon
        points={dataPoints(targetScores)}
      ...
      />
      <polygon
        points={dataPoints(currentScores)}
      ...
      {currentScores.map((s, i) => {
        ...
        return (
          <circle key={i} cx={p.x} cy={p.y} r={2.5} fill="#ba1a1a" stroke="white" strokeWidth={0.8} />
        );
      })}
