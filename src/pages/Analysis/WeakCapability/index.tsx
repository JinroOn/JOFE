        ...
        <header className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
          <div>
            ...
            <p className="text-on-surface-variant text-base sm:text-lg leading-relaxed">
              목표하신 <span className="font-semibold text-secondary">'컴퓨터공학'</span> 전공 대비 보완이 필요한 상위 3가지 역량을 AI가 분석했습니다.<br className="hidden sm:block" />
        ...
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {ROADMAP_WEEKS.map((w) => (
              <div
                key={w.week}
                className={`rounded-[14px] border px-4 py-3.5 flex items-start gap-3 ${statusStyle[w.status]}`}
              >
                <span className="material-symbols-outlined text-[20px] mt-0.5 shrink-0">
                  {statusIcon[w.status]}
                </span>
                <div>
                  <p className="text-xs font-bold opacity-60">WEEK {w.week}</p>
        ...
        <div>
          ...
          <div className="flex gap-4 overflow-x-auto pb-3 [&::-webkit-scrollbar]:hidden">
            {VIDEOS.map((v, i) => (
              <div
                key={i}
                className="shrink-0 w-52 sm:w-60 bg-surface-container-lowest rounded-[14px] border border-outline-variant/10 shadow-sm hover:shadow-md hover:-translate-y-1 transition-all cursor-pointer overflow-hidden"
              >
                <div className="h-36 bg-gradient-to-br from-primary-container/80 to-secondary flex items-center justify-center">
